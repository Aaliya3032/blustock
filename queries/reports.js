...
export async function createWatchReport(data){
   try {
     ...
     const modeule = await Module.findById(data.moduleId)
     const lessonIdsToCheck = modeule.lessonIds;
    ...
    const compltedModule = report.totalCompletedModeules
    const completedModuleCount = compltedModule?.length ?? 0
